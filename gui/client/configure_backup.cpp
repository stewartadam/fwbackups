...
configBackupsDialog::configBackupsDialog(QWidget *parent, int configType) {
  setupUi(this);
  // This is required to force a refresh
  advancedMode = true;
  this->setAdvancedMode(not advancedMode);
  // Type must be set after calls to the set*Mode functions
  type = configType;
  this->setType(type);
  // By default, we are creating a set
  originalSetName = "";
  
  // Need to set first tab & hide tab bar
  configurationTabs->setCurrentIndex(0);
  configurationTabs->tabBar()->hide();
  // Configure button on button
  finishButton->hide();
  nextButton->setDefault(true);
  
  
  timeSimpleFrequencyCombo->setCurrentIndex(2); // default to "Weekly" frequency
  // Forces the removal of the scheduling tab which is present at initialization
...
void configBackupsDialog::on_profileRecurringRadio_toggled(bool checked) {
  int tabIndex;
  tabIndex = 1;
  if (checked) {
    configurationTabs->insertTab(tabIndex, schedulingTab, tr("Scheduling"));
  } else {
    configurationTabs->removeTab(tabIndex);
  }
}
...
bool configBackupsDialog::loadConfiguration(QString setName) {
  ...
  serverConnectionCombo->setCurrentIndex( config->value("KeypackIndex").toInt() );
...
bool configBackupsDialog::saveConfiguration(QString setName) {
  ...
  config->beginGroup("Destination");
  config->setValue("Type", TYPE_DESTINATION_REMOTE);
  config->setValue("KeypackIndex", serverConnectionCombo->itemData(serverConnectionCombo->currentIndex()));
  config->endGroup(); // Destination
...
void configBackupsDialog::on_presetBookmarksProgramsButton_clicked() {
  chooseProgramsDialog *cpwindow = new chooseProgramsDialog(this);
 ...
 ************* Choose Programs *************
 *******************************************/
chooseProgramsDialog::chooseProgramsDialog(QWidget *parent) {
